...
const Main = ({ updateValue, data, event }) => {
  ...
  const [show, setShow] = useState(localStorage.getItem("yearState") === "true")

  const handleClick = () => {
    setShow(true)
    localStorage.setItem("yearState", true)
  }
...
const handelColor = (item, state) => {
  ...
  if (state === "failed" && !failedUnits.includes(item.name))
  {
    console.log(item.name)
    setFailedUnits(list => [...list, item.name])
  }
  ...
  {
    console.log(item.name)
    const index1 = failedUnits.indexOf(item.name)
    console.log(failedUnits[index1])

    if (failedUnits[index1] === item.name)
    {
            ...
            <div className="homecon">
              ...
              {data.slice(8, 16).map((item, index) => {
                  ...
                  return item.id ? (
                    ...
                    <div
                      key={index + new Date()}
                      onClick={() => history(`/search/${index}`,{state: {show}})}
                      >
